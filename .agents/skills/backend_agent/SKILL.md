...
# 🚀 Mandatory Protocol: CodeGraph First & Build Check
...
3. **CẤM CHẠY `go test`:** Tuyệt đối KHÔNG chạy `go test` hoặc `go test ./...`. Chỉ dùng `go build ./...` để kiểm tra lỗi biên dịch sau khi sửa code.
