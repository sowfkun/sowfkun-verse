...
const INFRA_SAMPLES: Record<InfraStack, SampleSpec> = {
  redis: {
    watch: {
      id: 'w-redis-demo',
      name: 'Sowfkun — RAM cạn kiệt (>92%)',
      enabled: true,
      stack: 'redis',
      connectionId: 'sowfkun-redis',
      connectionLabel: 'Sowfkun Redis',
      metric: 'memUsedPct',
      op: 'gt',
      threshold: 92,
      everySec: 30,
      ...
      severity: 'critical',
      tags: ['redis', 'sowfkun'],
      note: 'Redis này cấp session và cache cho Sowfkun Verse.',
    },
  ...
  mongo: {
    watch: {
      id: 'w-mongo-demo',
      name: 'Primary DB — replication lag (>10s)',
      enabled: true,
      stack: 'mongo',
      connectionId: 'sowfkun-mongo-01',
      connectionLabel: 'Primary Mongo',
      metric: 'replLagSec',
      op: 'gt',
      threshold: 10,
      everySec: 60,
  ...
  kafka: {
    watch: {
      ...
      enabled: true,
      stack: 'kafka',
      connectionId: 'sowfkun-kf-01',
      connectionLabel: 'Kafka Sowfkun',
  ...
  pg: {
    watch: {
      id: 'w-pg-demo',
      name: 'Postgres — mất kết nối',
      enabled: true,
      stack: 'pg',
      connectionId: 'sowfkun-pg-01',
      connectionLabel: 'Postgres DB',
      metric: 'up',
      op: 'lt',
      threshold: 1,
      everySec: 60,
    ...
    address: '127.0.0.1:5432/sowfkun',
    breach: 0,
    ok: 1,
  },
};
