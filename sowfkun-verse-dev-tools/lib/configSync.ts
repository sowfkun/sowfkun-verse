...
export async function setup(): Promise<SetupResult> {
  ...
  if (!await exists(MACHINE_FILE)) {
    const devboxRoot = process.cwd();          // app đang chạy ở đây
    const toolRoot = path.dirname(devboxRoot);
    const guesses = [
      path.join(path.dirname(toolRoot), 'sources'),
      path.join(path.dirname(devboxRoot), 'sources'),
      path.join(path.dirname(devboxRoot), 'projects'),
      path.dirname(devboxRoot),
    ];
    let sourcesRoot = '';
