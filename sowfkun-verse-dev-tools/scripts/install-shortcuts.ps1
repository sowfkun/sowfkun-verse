# Tao shortcut "Sowfkun Verse DevTools" ngoai Desktop + trong Start Menu (tim bang nut Windows),
# VA dang ky "Open with -> Sowfkun Verse DevTools" khi chuot phai file .xlsx/.csv/.docx/.md/
...
foreach ($lnk in $links) {
  ...
  $sc.Description = 'Sowfkun Verse DevTools desktop app'
  $sc.Save()
  Write-Host "Da tao: $lnk"
}

# -- "Open with -> Sowfkun Verse DevTools" khi chuot phai mot file -----------------------
#
# Cach lam: khai MOT ProgID (SowfkunVerseDevTools.Document) mo ta "mo file bang app nay",
...
$progId  = 'SowfkunVerseDevTools.Document'
...
Set-ItemProperty -Path "$classes\$progId" -Name '(Default)' -Value 'Tai lieu Sowfkun Verse DevTools'
